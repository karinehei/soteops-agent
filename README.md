...
Recruiter-facing path: [`docs/walkthrough/`](docs/walkthrough/README.md). Scenario pages, GIFs, and posters live in the repository. Animated GIFs are captured from the **local** app with fake LLM/embeddings. This presentation **replaces public deployment**. Do not expect a Vercel/Render/Neon/Azure/GitHub Pages visitor workspace.

Spoken 3–5 min local script (historical live demo notes, still valid for a machine in front of you): [`docs/demo-script.md`](docs/demo-script.md). Capture commands: [`docs/walkthrough/recording.md`](docs/walkthrough/recording.md). Must include:
...
3. **Timeout reconciliation** — lost-response forward still yields one mock record via the same idempotency key (the UI reconciles inside that request; a lingering `unknown` badge is backend/test coverage, not a parked screen).
...
| Recruiter walkthrough (`docs/walkthrough/`) | **Documented**; local GIFs recorded |
