...
7. Create a synthetic access request. The proposal shows extracted fields with input excerpts, deterministic rule findings, retrieved SYNTHETIC citations, and a fake-provider label. Human review is still required. An edit while the case is still eligible for review invalidates the previous proposal hash (edits are blocked after approve while a submission is pending, in flight, unknown, forwarding, or forwarded).
